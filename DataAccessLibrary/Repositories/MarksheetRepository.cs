...
namespace DataAccessLibrary.Repositories
{
    ...
    public class MarksheetRepository : IMarksheetRepository
    {
		...
		public MarksheetDTO GetMarksheetDetails(string psMsheet)
		{
            ...
            //see if below PS_EKS_DAT & VraestelKode gets filled
        ...
        public string GetMarksheetNum(string TextScan)
        {
            /* 033392363-3 exists */
            /* 033392363-7 no matches */

            string[] parts = TextScan.Split('-');
            if (parts.Length != 2)
            {
                return "TextScan must be in the format MSheet9-Msheet_Laaste_Syfer";
            }

            string msheet9 = parts[0] + "-";
            string msheetLaasteSyfer = parts[1];

            var markSheetExists = dbContext.EKS_PUNTESTATE
           .Where(e => e.MSheet9 == msheet9 && e.Msheet_Laaste_Syfer == msheetLaasteSyfer)
           .Select(e => new { e.MSheet9, e.Msheet_Laaste_Syfer })
           .FirstOrDefault();

			if (markSheetExists != null)
			{
				return markSheetExists.MSheet9 + markSheetExists.Msheet_Laaste_Syfer + " ✔";
			}
			else
			{
				return $"No matches found for {TextScan} ❌";
